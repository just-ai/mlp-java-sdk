...
abstract class MlpServiceBase<F : Any, FC : Any, P : Any, C : Any, R : Any>(
    ...
    /**
     * Creates a DeferredBilling object for working with deferred billing.
     *
     * This method automatically sets necessary headers in ThreadLocal,
     * which will be added to the service's first response. Gateway will save this data
     * and use it when processing subsequent DeferredBillingChargeRequestProto.
     *
     * Usage example:
     * ```kotlin
     * override suspend fun predict(req: Request): Response {
     *     val deferred = createDeferredBilling(
     *         billingDetails = mapOf("model" to "gpt-4", "operation" to "completion")
     *     )
     *
     *     // Start background task
     *     CoroutineScope(Dispatchers.IO).launch {
     *         processAsync(deferred.requestId)
     *     }
     *
     *     return Response("Processing started")
     * }
     *
     * private suspend fun processAsync(billingId: String) {
     ...
     * ```
     *
     * @param requestId Unique request ID (UUID generated by default)
     * @return DeferredBilling object for subsequent charge sending
     */
    fun createDeferredBilling(requestId: String = java.util.UUID.randomUUID().toString()) =
        DeferredBilling(requestId, sdk)

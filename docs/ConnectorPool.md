...
3. **gracefulShutdown()**
   ...
   Выполняет грациозное завершение работы пула коннекторов и **дожидается** каждого
   коннектора: пул считается остановленным, только когда все они отправили `stopServing`,
   слили активные запросы в пределах `MLP_GRACEFUL_SHUTDOWN_CONNECTOR_MS` и закрыли стримы.
